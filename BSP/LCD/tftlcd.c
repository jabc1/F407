...
{   
	...
	LCD_CTRL->LCD_REG=regval;//写入要写的寄存器序号	 
}
//写LCD数据
//data:要写入的值
...
{	
	LCD_CTRL->LCD_REG = LCD_Reg;		//写入要写的寄存器序号	 
	LCD_CTRL->LCD_RAM = LCD_RegValue;//写入数据	    		 
}	   
//读寄存器
//LCD_Reg:寄存器地址
//返回值:读到的数据
...
{							    
	LCD_CTRL->LCD_RAM = RGB_Code;//写十六位GRAM
}
...
{          
 	...
 	}else
	{
		...
		for(i=sy;i<=ey;i++)
		{
			...
			LCD_WriteRAM_Prepare();     			//开始写入GRAM	  
			for(j=0;j<xlen;j++)LCD_CTRL->LCD_RAM=color;	//显示颜色 	    
		}
	}	 
}  
//在指定区域内填充指定颜色块			 
...
{  
 	...
 	for(i=0;i<height;i++)
	{
		...
		LCD_WriteRAM_Prepare();     //开始写入GRAM
		for(j=0;j<width;j++)LCD_CTRL->LCD_RAM=color[i*width+j];//写入数据 
	}		  
}  
//画线
//x1,y1:起点坐标
